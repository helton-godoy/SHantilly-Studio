...
QJsonObject ProjectSerializer::serializeWidget(QWidget *widget)
{
    ...
    // Lista de propriedades de interesse (mesma lógica do ScriptGenerator, mas expansível)
    QStringList keys = {"text", "title", "checked", "value", "minimum", "maximum", "visible", "enabled"};
    ...
    for (const QString &key : keys) {
        ...
        if (val.isValid()) {
             ...
             if (val.typeId() == QMetaType::Bool) props[key] = val.toBool();
             else if (val.typeId() == QMetaType::Int) props[key] = val.toInt();
             else props[key] = val.toString();
        }
    }
    ...
    if (widget->layout()) {
        for (int i = 0; i < widget->layout()->count(); ++i) {
            if (QWidget *w = widget->layout()->itemAt(i)->widget()) childWidgets.append(w);
        }
    } else {
        for (QObject *child : widget->children()) {
            if (QWidget *w = qobject_cast<QWidget*>(child)) childWidgets.append(w);
        }
    }
...
bool ProjectSerializer::load(const QString &filename, IStudioWidgetFactory *factory, QList<QWidget*> &outWidgets)
{
    QFile file(filename);
...
QWidget* ProjectSerializer::deserializeWidget(const QJsonObject &json, IStudioWidgetFactory *factory)
{
    ...
    // Como estamos recriando, o caller vai decidir onde colocar (ex: adicionar ao Canvas).
