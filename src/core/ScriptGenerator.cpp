...
QString ScriptGenerator::generate(QWidget* root) {
    ...
    QString script;
    script += "#!/bin/bash\n";
    script += "# Generated by SHantilly Studio\n\n";
    ...
    // Assumindo que 'root' é o container onde os widgets foram soltos (ex:
    // Canvas)
...
void ScriptGenerator::processWidget(QWidget* widget, QString& script) {
    ...
    QString type = getSHantillyType(widget);
    if (type.isEmpty())
        return; // Não sabemos exportar este tipo
    ...
    if (textProp.isValid() && !textProp.toString().isEmpty())
        title = textProp.toString();
    else if (titleProp.isValid() && !titleProp.toString().isEmpty())
    ...
    if (name.isEmpty())
        name = "unnamed_" + QString::number((quintptr) widget);
    ...
    bool isContainer =
        (type == "window" || type == "groupbox" || type == "frame" || type == "tabs" ||
         type == "page" || type == "hboxlayout" || type == "vboxlayout" || type == "gridlayout" ||
         type == "formlayout" || type == "scrollarea");
    ...
    if (type == "groupbox" || type == "frame" || type == "tabs" || type == "page" ||
        type == "hboxlayout" || type == "vboxlayout" || type == "gridlayout" ||
        type == "formlayout" || type == "scrollarea") {
        script += QString("end %1\n").arg(type);
    }
}

QString ScriptGenerator::getSHantillyType(QWidget* widget) {
    ...
    if (cls.contains("Button")) {
        if (cls.contains("Check"))
            return "checkbox";
        if (cls.contains("Radio"))
            return "radiobutton";
        return "button";
    }
    if (cls.contains("Label"))
        return "label";
    if (cls.contains("LineEdit"))
        return "textbox";
    if (cls.contains("TextEdit"))
        return "textedit";
    if (cls.contains("SpinBox"))
        return "spinbox";
    if (cls.contains("Slider"))
        return "slider";
    if (cls.contains("ComboBox"))
        return "combobox";
    if (cls.contains("ListWidget") || cls.contains("ListView"))
        return "listbox";
    ...
    if (cls.contains("Table"))
        return "table";
    if (cls.contains("Calendar"))
        return "calendar";
    if (cls.contains("Line"))
        return "separator";

    if (cls.contains("ChartWidget"))
        return "chart";
...
QString ScriptGenerator::getPropertiesString(QWidget* widget) {
    ...
    if (widget->property("echoMode").isValid() &&
        widget->property("echoMode").toInt() == 2) // Password
        parts << "password";
    ...
    for (auto it = events.begin(); it != events.end(); ++it) {
        ...
        for (const QJsonValue& actionVal : actions) {
            ...
            if (type == "shell") {
                ...
                commands << QString("SHantilly set %1 %2 \"%3\"").arg(prop, target, value);
            } else if (type == "query") {
                QString target = action["target"].toString();
