...
public:
    AddWidgetCommand(Canvas* canvas, QWidget* widget, QWidget* parent = nullptr,
        ...
        : QUndoCommand(parentCmd), m_canvas(canvas), m_widget(widget), m_parent(parent) {
...
public:
    ...
    DeleteWidgetCommand(Canvas* canvas, QList<QWidget*> widgets, QUndoCommand* parent = nullptr)
        ...
        for (QWidget* w : widgets) {
            ...
            info.index =
                (info.parent && info.parent->layout()) ? info.parent->layout()->indexOf(w) : -1;
...
public:
    PropertyChangeCommand(QWidget* target, const QString& propertyName, const QVariant& oldValue,
    ...
    void undo() override { m_target->setProperty(m_propertyName.toUtf8().constData(), m_oldValue); }

    void redo() override { m_target->setProperty(m_propertyName.toUtf8().constData(), m_newValue); }
...
public:
    MoveWidgetCommand(QWidget* widget, QWidget* newParent, int newIndex = -1,
        ...
        setText(QString("Move %1 to %2")
                    .arg(widget->objectName(), newParent ? newParent->objectName() : "Root"));
    }
...
public:
    ...
    GroupWidgetsCommand(Canvas* canvas, IStudioWidgetFactory* factory, StudioController* controller,
                        const QList<QWidget*>& widgets, const QString& containerType,
                        QUndoCommand* parent = nullptr)
        : QUndoCommand(parent),
          m_canvas(canvas),
          m_factory(factory),
    ...
    void redo() override {
        if (!m_container) {
            m_container =
                m_factory->createWidget(m_containerType, m_containerType.toLower() + "_group");
