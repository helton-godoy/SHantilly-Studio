...
ActionEditor::ActionEditor(QWidget* parent) : QWidget(parent) {
    ...
    connect(m_eventCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this](int) { updateActionsList(); });
    ...
    connect(m_actionsList, &QListWidget::currentRowChanged, this, &ActionEditor::onActionSelected);
    ...
    connect(removeBtn, &QPushButton::clicked, this, &ActionEditor::onRemoveAction);
    ...
    m_actionType->addItems(
        {"Comando Shell", "Script Externo", "Modificar Widget", "Consultar Widget", "Callback"});
    typeLayout->addWidget(m_actionType, 1);
    detailsLayout->addLayout(typeLayout);

    connect(m_actionType, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &ActionEditor::onActionTypeChanged);
    ...
    connect(m_commandLine, &QLineEdit::textChanged, this, &ActionEditor::onCommandChanged);
    ...
    connect(exportBtn, &QPushButton::clicked, this, [this]() {
        ...
        QString fileName =
            QFileDialog::getSaveFileName(this, "Exportar Ações", QString(), "JSON (*.json)");
        if (fileName.isEmpty())
            return;
        ...
        if (file.open(QIODevice::WriteOnly)) {
            ...
            file.close();
            QMessageBox::information(this, "Exportar", "Ações exportadas com sucesso!");
        }
    });
    ...
    connect(importBtn, &QPushButton::clicked, this, [this]() {
        QString fileName =
            QFileDialog::getOpenFileName(this, "Importar Ações", QString(), "JSON (*.json)");
        if (fileName.isEmpty())
            return;
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            if (doc.isObject()) {
                ...
                updateActionsList();
                saveActionsToWidget();
                QMessageBox::information(this, "Importar", "Ações importadas com sucesso!");
            } else {
...
void ActionEditor::saveActionsToWidget() {
    ...
    QJsonDocument doc(m_actionsData);
    m_target->setProperty("SHantilly_actions", QString(doc.toJson(QJsonDocument::Compact)));
...
void ActionEditor::onActionTypeChanged(int index) {
    ...
    m_commandLine->parentWidget()->findChild<QLabel*>()->setVisible(isShellOrScript);
...
void ActionEditor::onTestAction() {
    ...
    if (type == "shell" || type == "script") {
        ...
        if (level == Dangerous) {
            ...
            int ret =
                QMessageBox::warning(this, "⚠️ Aviso de Segurança",
        ...
        QMessageBox::information(this, "Teste",
                                 "Ações do tipo '" + type + "' só podem ser testadas no Preview.");
    }
}
...
ActionEditor::SecurityLevel ActionEditor::analyzeCommand(const QString& cmd) {
    ...
    QStringList warningPatterns = {"sudo",    "su ",      "rm ",       "mv ",    "cp ",
                                   ">",       ">>",       "|",         "kill",   "pkill",
                                   "killall", "shutdown", "reboot",    "apt",    "yum",
                                   "dnf",     "pacman",   "systemctl", "service"};
...
QString ActionEditor::getSecurityWarning(const QString& cmd) {
    ...
    return warnings.isEmpty() ? "Nenhum risco específico identificado" : warnings.join("\n");
}
...
void ActionEditor::updateSecurityIndicator() {
    ...
    switch (level) {
        case Safe:
            ...
            break;
        case Warning:
            m_securityLabel->setText("⚠️ " + getSecurityWarning(cmd).split("\n").first());
            m_securityLabel->setStyleSheet("background-color: #4d4d1a; color: #FFD700; "
                                           "padding: 5px; border-radius: 3px;");
            break;
        case Dangerous:
            m_securityLabel->setText("❌ PERIGOSO: " + getSecurityWarning(cmd).split("\n").first());
