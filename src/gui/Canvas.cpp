    ...
    if (!selected.isEmpty()) {
        if (selected.size() == 1) {
            ...
            if (SHantillyType == "tabwidget") {
                ...
                menu.addAction("Remove Current Page", [this, w]() { emit requestRemovePage(w); });
                menu.addSeparator();
            }
        }
...
void Canvas::dropEvent(QDropEvent* event) {
    ...
    if (newWidget) {
        ...
        qDebug() << "[DROP] Initial targetContainer:"
                 << (targetContainer ? targetContainer->objectName() : "nullptr") << "type:"
                 << (targetContainer ? targetContainer->property("SHantilly_type").toString() : "");
        ...
        if (m_controller) {
            m_controller->undoStack()->push(new AddWidgetCommand(this, newWidget, targetContainer));
        } else {
            // Fallback sem Undo
...
QWidget* Canvas::findContainerAtPos(const QPoint& pos) {
    ...
    while (child && child != this) {
        ...
        if (SHantillyType.contains("layout") || SHantillyType == "groupbox" ||
            SHantillyType == "frame" || SHantillyType == "scrollarea" || SHantillyType == "tabs" ||
            SHantillyType == "page") {
            return child;
        }
