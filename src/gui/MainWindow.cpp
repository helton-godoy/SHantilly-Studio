...
MainWindow::MainWindow(QWidget* parent)
    : QMainWindow(parent), m_dockToolbox(nullptr), m_toolbox(nullptr), m_toolboxStyle(0) {
    ...
    connect(m_canvas, &Canvas::requestAddPage, this, &MainWindow::onAddPageRequested);
    connect(m_canvas, &Canvas::requestRemovePage, this, &MainWindow::onRemovePageRequested);
...
void MainWindow::setupUI() {
    ...
    connect(styleGroup, &QActionGroup::triggered, this,
            [this](QAction* action) { onToolboxStyleChanged(action->data().toInt()); });
...
void MainWindow::onGroupRequested(const QString& containerType) {
    ...
    m_controller->undoStack()->push(
        new GroupWidgetsCommand(m_canvas, m_factory, m_controller, selected, containerType));
...
void MainWindow::onRunClicked() {
    ...
    m_previewLog->clear();
    m_previewLog->append(
        QString("<b>Starting Preview...</b> [%1]").arg(QTime::currentTime().toString()));
...
void MainWindow::onSaveClicked() {
    QString fileName =
        QFileDialog::getSaveFileName(this, "Save Project", "", "SHantilly Project (*.sbxproj)");
    if (fileName.isEmpty())
        return;
...
void MainWindow::onOpenClicked() {
    QString fileName =
        QFileDialog::getOpenFileName(this, "Open Project", "", "SHantilly Project (*.sbxproj)");
    if (fileName.isEmpty())
        return;
...
void MainWindow::onAddPageRequested(QWidget* tabs) {
    if (!tabs)
        return;
    QString name = QString("%1_page%2").arg(tabs->objectName()).arg(tabs->children().size());
